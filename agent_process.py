...
class AgentProcessFactory:
    def __init__(self, agent_process_log_mode = None):
        ...
        self.thread = Thread(target=self.deactivate_singular_agent)
    ...
    def deactivate_singular_agent(self):
        # Deactivate the singular agent by passing its pid to the deactivate_agent_process method
        if self.pid is not None:
            self.deactivate_agent_process(self.pid)
            self.pid = None  # Reset the pid
        else:
            print("No agent process is running.")
