...
class ReactAgentAttack(BaseAgent):
    ...
    def run(self):
        ...
        if workflow:
            ...
            for i, step in enumerate(workflow):
                ...
                if (
                    "It is very important" in message
                    or "in light of extant literature" in message
                    or "since there is" in message
                ):
                    print(f"[ReactAgentAttack] Skipping non-actionable step: {message}")
                    continue
    ...
    def load_agent_json(self):
        ...
        # Load from the JSONL task file directly to avoid using undefined 'all_agent_configs'
        try:
            with open("data/stockagent_task.jsonl", "r") as f:
                all_agent_configs = [json.loads(line.strip()) for line in f if line.strip()]

            print("Looking for:", self.agent_name)
            for c in all_agent_configs:
                print(c['agent_name'], c.get('agent_path', 'N/A'))

            for item in all_agent_configs:
                if item["agent_name"] == self.agent_name:
                    selected_pot_msg_json = item.get("pot_messages", [None])[0]
                    break
        except Exception as e:
            raise RuntimeError(f"Error loading agent config: {e}")

        if selected_pot_msg_json is None:
            raise ValueError(f"Could not find pot_messages for agent: {self.agent_name}")

        # Load config JSON for tool references
        with open(agent_path, "r") as f:
            agent_config_json = json.load(f)

        tools = agent_config_json.get("tools", [])
        self.tool1 = tools[0].split('/')[-1] if len(tools) > 0 else None
        self.tool2 = tools[1].split('/')[-1] if len(tools) > 1 else None

    
        required_keys = ["Task1", "msg1", "msg2", "Task2", "msg3", "msg4"]
        missing_keys = [key for key in required_keys if key not in selected_pot_msg_json]
        if missing_keys:
            raise KeyError(f"Missing required key(s) in selected_pot_msg_json: {missing_keys}")
